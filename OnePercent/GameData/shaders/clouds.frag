...
float cloud_noise(float diff)
{
	float edgeWidth_i = 20.0;

	float n = (snoise(vec3(gl_TexCoord[0].st * 10.0 + vec2(diff, diff), time)) + 1.0) / 2.0;

	float edge1 = 1.0 - clamp(gl_TexCoord[0].s * edgeWidth_i, 0.0, 1.0);
	float edge2 = 1.0 - clamp((1.0 - gl_TexCoord[0].s) * edgeWidth_i, 0.0, 1.0);
	float edge3 = 1.0 - clamp(gl_TexCoord[0].t * edgeWidth_i, 0.0, 1.0);
	float edge4 = 1.0 - clamp((1.0 - gl_TexCoord[0].t) * edgeWidth_i, 0.0, 1.0);

	n *= (1.0 - clamp(edge1 + edge2 + edge3 + edge4, 0.0, 1.0));
...
{ 
	...
	for (int i=0; i<MAX_LIGHTS; i++)
	{
		...
		vec4 Idiff = gl_FrontLightProduct[i].diffuse * max(dot(N,L), 0.0) * intensity;
