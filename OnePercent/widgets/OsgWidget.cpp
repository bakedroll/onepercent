...
class EventAdapter : public osgGA::GUIEventAdapter
{
  ...
  EventAdapter(QMouseEvent* event, int height) : osgGA::GUIEventAdapter()
  {
    setX(event->x());
    setY(height - event->y() - 1);
    setButton(c_buttons[event->button()]);
  }
  EventAdapter(QMouseEvent* event, int button, int height) : osgGA::GUIEventAdapter()
  {
    setX(event->x());
    setY(height - event->y() - 1);
    setButton(c_buttons[button]);
  }
  EventAdapter(QWheelEvent* event) : osgGA::GUIEventAdapter()
  {
    setScrollingMotion(event->delta() < 0 ? SCROLL_DOWN : SCROLL_UP);
  }
  EventAdapter(QKeyEvent* event) : osgGA::GUIEventAdapter()
  {
...
struct OsgWidget::Impl
{
  Impl(OsgWidget* b, const osg::ref_ptr<osgViewer::CompositeViewer>& viewer)
    : base(b), viewer(viewer), isInitialized(false)
  {
...
OsgWidget::OsgWidget(const osg::ref_ptr<osgViewer::CompositeViewer>& viewer, QWidget* parent, Qt::WindowFlags f)
  : QGLWidget(parent, nullptr, f), InputManager(), m(new Impl(this, viewer))
{
...
{
  if (m->compositor)
    m->compositor->renderVirtualOverlays();
...
void OsgWidget::resizeGL(int width, int height)
{
  ...
  geode->addDrawable(osg::createTexturedQuadGeometry(
          osg::Vec3f(-factor, -1.0f, 0.0f), osg::Vec3f(2.0f * factor, 0.0f, 0.0f), osg::Vec3f(0.0f, 2.0f, 0.0f)));
